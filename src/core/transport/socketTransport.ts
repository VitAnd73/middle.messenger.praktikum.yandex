export default class SocketTransport {
    // order per WebSocket: readyState property
    private SocketStates = ['CONNECTING', 'OPEN', 'CLOSING', 'CLOSED'];
    ...
    public getState = () => {
        if (!this.socket) return this.SocketStates[3];
        return this.SocketStates[this.socket.readyState];
    }
